...
#slope, elevation, elev to HDHyd, elev VD hyd, elevHdRoad, hRoadways, fire hyd, hill 9 AM
   ...
   # p3=(1 / (math.sqrt(2 * math.pi) * contData.get("aspect")[0][1])) * math.exp(
    ...
    for k in pl:
        ...
        ct1=ct1*k
	# Using log function for the probabilities we can add the results instead of multiplying
...
if __name__ == "__main__":
    ...
    # total records count
    n = data.count() -1  # minus one to ignore the label row in the record
	...
	alpha=1
	totalSoilType=40
	totalWildernessType=4
    likeProbSt = numpy.zeros((7, 40), dtype=float)
    for ct in range(7):
        for st in range(40):
           # smoothening by using alpha and adding denominator by total soilTypes
             likeProbSt[ct][st] = (float(conditionalSoilTpCount[ct][st]+alpha) / ((coverTypeCount[ct])+totalSoilType))
           # else:
    ...
    for ct in range(7):
        for st in range(4):
           # if conditionalWilCount[ct][st] != 0 and coverTypeCount[ct] != 0:
             likeProbWil[ct][st] = (float(conditionalWilCount[ct][st]+alpha) / ((coverTypeCount[ct])+totalWildernessType))
           # else:
